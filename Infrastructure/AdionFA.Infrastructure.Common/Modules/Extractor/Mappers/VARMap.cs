...
{
    public sealed class VARMap : ClassMap<VAR>
    {
        public VARMap()
        {
            Map(m => m.PriceType).Index(0).TypeConverter<VARValueTransform<Enum>>();
            Map(m => m.OptInTimePeriod).Index(1);
        }
    }

    public class VARValueTransform<T> : DefaultTypeConverter
    {
