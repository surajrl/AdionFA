...
namespace Adion.FA.Infrastructure.Common.Infrastructures.StrategyBuilder.Services
{
    public class StrategyBuilderService : IStrategyBuilderService
    {
        ...
        public StrategyBuilderModel BacktestBuild(
            string label, List<string> node, ConfigurationBaseDTO config, IEnumerable<Candle> data)
        {
            try
            {
                BacktestModel bkmIS = BacktestExecute(label, config.FromDateIS.Value, config.ToDateIS.Value, 
                    node.ToList(), data, config.CurrencyPeriodId, config.Variation);

                BacktestModel bkmOS = BacktestExecute(label, config.FromDateOS.Value, config.ToDateOS.Value, 
                    node.ToList(), data, config.CurrencyPeriodId, config.Variation);
                ...
                stb.WinningStrategy = ApplyWinningStrategyRules(bkmIS, bkmOS, config, stb.VariationPercent, stb.Progressiveness);
        ...
        public BacktestModel BacktestExecute(
            string label, DateTime fromDate, DateTime toDate, List<string> node,
            IEnumerable<Candle> data, int periodId, decimal variation = 0)
        {
            try
            {
