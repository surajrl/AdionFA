...
{
    public sealed class RSIMap : ClassMap<RSI>
    {
        public RSIMap()
        {
            Map(m => m.PriceType).Index(0).TypeConverter<RSIValueTransform<Enum>>();
            Map(m => m.OptInTimePeriod).Index(1);
        }
    }

    public class RSIValueTransform<T> : DefaultTypeConverter
    {
