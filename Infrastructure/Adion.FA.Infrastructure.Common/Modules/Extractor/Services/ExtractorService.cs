...
using Adion.FA.Infrastructure.Common.Extractor.Mappers;
...
namespace Adion.FA.Infrastructure.Common.Extractor.Services
{
    public class ExtractorService : InfrastructureServiceBase, IExtractorService
    {
        ...
        public List<IndicatorBase> ExtractorExecute(
            DateTime fromDate, DateTime toDate, List<IndicatorBase> indicators, IEnumerable<Candle> candles, int periodId, decimal variation = 0)
        {
            try
            {
                if (periodId == 0)
                ...
                List<IndicatorBase> extractions = new List<IndicatorBase>(indicators);
                ...
                double[] max = (from c in data select c.max).ToArray();
                double[] close = (from c in data select c.close).ToArray();
                double[] min = (from c in data select c.min).ToArray();

                IEnumerable<Candle> candlesRange = from c in data
                                                   let dt = DateTimeHelper.BuildDateTime(periodId, c.date, c.time)
                ...
                ExtractorExecute(startIdx, endIdx, open, max, min, close, extractions, candlesRange, periodId);
        ...
        private void ExtractorExecute(
            int startIdx, int endIdx, double[] open, double[] high, double[] low, double[] close, List<IndicatorBase> indicators, IEnumerable<Candle> candlesRange, int periodId)
        {
            try
            {
                ...
                foreach (var indicator in indicators)
                {
                    ...
                    indicator.IntervalLabels = (from cr in candlesRange select new IntervalLabel 
                    {
                        Interval = DateTimeHelper.BuildDateTime(periodId, cr.date, cr.time),
                        Label = cr.label
                    }).ToArray();
                }
            }
