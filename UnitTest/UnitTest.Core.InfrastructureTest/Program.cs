...
namespace UnitTest.Core.InfrastructureTest
{
    ...
    public static class ProgramTest
    {
        public static void CreateHistory()
        {
            ...
            if (result.Count > 0)
            {
                var marketHistoryData = new HistoricalDataDTO
                {
                    MarketId = (int)MarketEnum.Forex,
                    CurrencyPairId = (int)CurrencyPairEnum.EURUSD,
                    TimeframeId = (int)TimeframeEnum.H1,
                    HistoricalDataDetails = Array.Empty<HistoricalDataDetailDTO>().ToList()
                };

                foreach (var item in result)
                {
                    marketHistoryData.HistoricalDataDetails.Add(new HistoricalDataDetailDTO
                    {
                        StartDate = item.Date,
                        StartTime = item.Time,
                        OpenPrice = item.Open,
                ...
                IoC.Get<IHistoricalDataAppService>().CreateHistoricalData(marketHistoryData);
            }
        }
        ...
        public static ResponseDTO CreateProjects()
        {
            ...
            if (pds.CreateDefaultWorkspace())
            {
                ...
                HistoricalDataDTO dataDTO = IoC.Get<IHistoricalDataAPI>().GetHistoricalData(
                            (int)MarketEnum.Forex, (int)CurrencyPairEnum.EURUSD, (int)TimeframeEnum.H1);
